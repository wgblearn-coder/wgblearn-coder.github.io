---
title: "消息队列怎么保证可靠：从支付订单到异步处理"
date: "2026-09-07"
description: "沿餐饮订单已支付后的真实请求链，讲清消息队列的模型、存储、可靠性、幂等、重试、选型与容量治理，并给出可落地的 Java 与 SQL 设计。"
tags: [消息队列, Kafka, RabbitMQ, RocketMQ, Java]
---

订单已经支付，收银台要尽快返回成功。经营统计还要汇总，积分要发放，短信或站内信要通知，失败也得能补偿。把这几件事塞进一次同步请求，下游任何一处变慢都会传回收银台；直接开线程“异步一下”，进程一崩，积分和通知又可能悄悄丢掉。

消息队列把这条链路的时间拆开，也把故障隔离开：订单服务提交一条可追踪的业务事件，队列负责保存，再按规则交给下游消费者。它不会自动带来业务上的恰好一次，也代替不了数据库事务、幂等约束和监控。下面一直沿用“订单已支付 → 经营统计、积分、通知”这个场景；容量、延迟和故障现象都是教学假设，不能当作线上实测结果。

可以先记住一个判断：业务事实要先可靠落地，再让不同消费者各自推进进度。发送、存储、消费、业务提交和外部调用之间的确认边界，要分别定义。
...
同步调用里，调用者要等被调用方返回。例如订单服务支付完成后调用积分服务 HTTP 接口，收到成功才返回。异步调用则把可以继续推进的事实交给中间层，自己先返回，积分服务稍后消费事件。这样主链路少受下游延迟影响，但失败会变成后续的重试、积压、告警和对账。
...
记录的 offset 是它在分区中的位置，提交的消费 offset 表示下一条要读的位置，不能把它当成某条业务记录的“已成功”标记。批量拉取 offset 10、11、12 时，10 失败而 11、12 完成，不能越过 10 提交 13；并发处理要维护完成集合和最小缺口，或按 key 串行化。
...
统计、积分和通知各自维护进度、重试和告警。统计短暂故障不该挡住积分；通知供应商限流时，也只应拖慢通知消费。事件用 `partitionKey=tenantId+orderId` 分区，可以让同一订单的支付、退款、取消出现在一个有序流里，但这不提供消费者之间的全局顺序。
...
fencing 处理的是这样的场景：实例 A 的租约过期并暂停，实例 B 重新领取、发布；A 恢复后如果还能把状态写成成功，就会覆盖 B 的新事实。token 充当单调栅栏，更新必须满足 `WHERE tenant_id=? AND event_id=? AND publish_token=?`。
...
## Kafka、RabbitMQ、RocketMQ 如何有条件地选
...
可以先画一条订单事件链，标出每个系统的事实、命令、确认和失败恢复，再手写 outbox、inbox、业务唯一约束和状态 CAS。接着用一个本地 broker 验证 batch、partition、consumer group、ACK、重试和重放。

再做容量算例，解释最老积压时间和背压阈值，演练 publisher 崩溃、消费者 ACK 丢失、外部 HTTP UNKNOWN、复制节点故障和死信回放。最后比较 Kafka、RabbitMQ、RocketMQ，把 Agent/RAG 的任务版本、权限过滤、取消和 token 限流映射到同一套边界上。

可靠性落在每一段的具体回答上：事实写在哪里，成功由谁确认，失败后怎样重试、去重、对账或重放。
  tenant_id     VARCHAR(64)  NOT NULL,
